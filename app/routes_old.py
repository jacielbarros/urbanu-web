...
def new_solicitation():
    ...
    if form.validate_on_submit():
        # Criar a solicitação sem imagem
        ...
        db.session.flush()  # Este comando gera o ID da solicitação
        solicitation.generate_protocol()
        db.session.commit()  # Salvar a solicitação com o protocolo gerado
...
def upload_images(solicitation_id):
    ...
    if request.method == 'POST':
        ...
        for file in files:
            if file:
                image_filename = f"{solicitation.protocol}_{file.filename}"
                image_path = os.path.join('app', 'static', 'uploads', image_filename)
                os.makedirs(os.path.dirname(image_path), exist_ok=True)
                file.save(image_path)
                # Aqui você pode adicionar o caminho da imagem ao banco de dados se precisar
                solicitation.image_path = image_path
        db.session.commit()
        flash('Imagens foram carregadas com sucesso!')
        return redirect(url_for('routes.solicitante_home'))
    return render_template('upload_images.html', solicitation=solicitation)



@bp.route('/admin/update/<int:solicitation_id>', methods=['GET', 'POST'])
@login_required
...
def solicitations():
    ...
    if filter_name:
        query = query.join(User).filter(User.full_name.ilike(f"%{filter_name}%"))
    if filter_status:
...
def view_solicitation(solicitation_id):
    solicitation = Solicitation.query.get_or_404(solicitation_id)
    return render_template('view_solicitation.html', title='Visualizar Solicitação', solicitation=solicitation)
